import { Schema, model, Document } from 'mongoose';

export interface INationTurnDocument extends Document {
  sessionId: string;
  nationId: string;
  officerLevel: number;
  turnIdx: number;
  action: string;
  arg?: Record<string, any>;
  brief?: string;
}

const NationTurnSchema = new Schema<INationTurnDocument>(
  {
    sessionId: { type: String, required: true },
    nationId: { type: String, required: true },
    officerLevel: { type: Number, required: true },
    turnIdx: { type: Number, required: true },
    action: { type: String, required: true },
    arg: { type: Schema.Types.Mixed },
    brief: { type: String },
  },
  { timestamps: true }
);

NationTurnSchema.index({ sessionId: 1, nationId: 1, officerLevel: 1, turnIdx: 1 });

export const NationTurnModel = model<INationTurnDocument>('NationTurn', NationTurnSchema);
