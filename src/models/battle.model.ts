...
export const BattleUnitSchema = new Schema({
  ...
  lastCommandSeq: { type: Number, default: 0 },
  lastCommandTime: { type: Number, default: 0 }
}, { _id: false });

const TurnActionSchema = new Schema({
  generalId: { type: Number, required: true },
  action: { type: String, enum: ['move', 'attack', 'skill', 'defend', 'retreat', 'fire', 'ambush', 'duel', 'stone', 'misinform', 'discord', 'confuse'], required: true },
  target: {
    x: { type: Number },
    y: { type: Number }
  },
...
const BattleSchema = new Schema({
  ...
  weather: { type: String, enum: Object.values(WeatherType), default: WeatherType.SUNNY },
  weatherNextChangeTick: { type: Number, default: 0 },
