...
export class JosaUtil {
   ...
   */
  static pick(noun: string, josa: string): string {
    if (!noun || !josa) {
      return josa || '';
    }
   ...
   * 명사에 조사 붙이기
   */
  static attach(noun: string, josa: string): string {
    const pickedJosa = this.pick(noun, josa);
  ...
  private static getLastChar(str: string): string {
    ...
    // 간단한 버전: 마지막 글자만 가져옴 (복합 문자는 고려 안 함)
