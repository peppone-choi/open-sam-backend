...
router.get('/server/basic-info', async (req: Request, res: Response) => {
  try {
    ...
    const serverInfo = {
      ...
      year: sessionData.game_env?.year || sessionData.year || sessionData.current_year || session.year || 184,
      month: sessionData.game_env?.month || sessionData.month || sessionData.current_month || session.month || 1,
