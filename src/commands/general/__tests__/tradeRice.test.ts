...
describe('TradeRiceCommand', () => {
  describe('기본 구조 테스트', () => {
    it('클래스가 정의되어 있어야 함', () => {
      expect(TradeRiceCommand).toBeDefined();
    });

    it('getName() 메서드가 있어야 함', () => {
      expect(typeof TradeRiceCommand.getName).toBe('function');
      const name = TradeRiceCommand.getName();
  ...
  describe('인스턴스 생성 테스트', () => {
    it('유효한 인자로 인스턴스를 생성할 수 있어야 함', () => {
      ...
      expect(command instanceof TradeRiceCommand).toBe(true);
    });
  });
