...
describe('StepDownCommand', () => {
  describe('기본 구조 테스트', () => {
    it('클래스가 정의되어 있어야 함', () => {
      expect(StepDownCommand).toBeDefined();
    });

    it('getName() 메서드가 있어야 함', () => {
      expect(typeof StepDownCommand.getName).toBe('function');
      const name = StepDownCommand.getName();
  ...
  describe('인스턴스 생성 테스트', () => {
    it('유효한 인자로 인스턴스를 생성할 수 있어야 함', () => {
      ...
      expect(command instanceof StepDownCommand).toBe(true);
    });
  });
