...
describe('GrantCommand', () => {
  describe('기본 구조 테스트', () => {
    it('클래스가 정의되어 있어야 함', () => {
      expect(GrantCommand).toBeDefined();
    });

    it('getName() 메서드가 있어야 함', () => {
      expect(typeof GrantCommand.getName).toBe('function');
      const name = GrantCommand.getName();
  ...
  describe('인스턴스 생성 테스트', () => {
    it('유효한 인자로 인스턴스를 생성할 수 있어야 함', () => {
      ...
      expect(command instanceof GrantCommand).toBe(true);
    });
  });
