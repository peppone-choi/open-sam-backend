...
export class FoundNationCommand extends GeneralCommand {
  ...
  public async run(rng: any): Promise<boolean> {
    ...
    if (!currentCity) {
      logger.pushGeneralActionLog(`도시 정보를 찾을 수 없습니다. <1>${date}</>`);
      return false;
    }

    const cityLevel = currentCity.level || 0;
    if (cityLevel < 5) {
      logger.pushGeneralActionLog(`이민족 영토 또는 소(小) 이상의 도시에서만 건국할 수 있습니다. <1>${date}</>`);
      return false;
    }
    ...
    try {
      ...
      await inheritanceService.recordActivity(userId, InheritanceKey.UNIFIER, 250);
    } catch (error: any) {
  ...
  public async exportJSVars(): Promise<any> {
    ...
    // 사용 가능한 국가 타입 (getNationTypeName을 사용)
    const nationTypes = {};
