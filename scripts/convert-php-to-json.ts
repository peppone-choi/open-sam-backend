/**
 * PHP 도시 데이터를 JSON 형식으로 변환
 * CityConstBase.php의 $initCity 배열을 cities.json 형식으로 변환
 */

// CityConstBase.php의 $initCity 데이터 (58-154줄)
const phpData = [
...
// 규모 매핑 - PHP의 $levelMap에 맞춤
// PHP: '수'=>1, '진'=>2, '관'=>3, '이'=>4, '소'=>5, '중'=>6, '대'=>7, '특'=>8
// 하지만 프론트엔드에서는 9='특', 10='경'이므로 PHP의 '특'은 9로 매핑
const levelMap: Record<string, number> = {
  '특': 9,  // PHP의 '특'은 레벨 8이지만, 프론트엔드에서는 9='특', 10='경'
  '대': 8,
  '중': 7,
  '소': 6,
  '진': 3,
...
phpData.forEach(([id, name]: any) => {
  cityNameToId[name] = id;
});

// JSON 변환
const jsonCities = phpData.map(([id, name, scale, pop, agri, comm, secu, def, wall, region, x, y, neighbors]: any) => {
  // neighbors를 이름 배열에서 ID 배열로 변환
  const neighborIds = (neighbors || []).map((neighborName: string) => {
    const neighborId = cityNameToId[neighborName];
    if (!neighborId) {
      console.warn(`⚠️  연결 도시를 찾을 수 없음: ${neighborName} (도시 ${id}: ${name})`);
    }
    return neighborId;
  }).filter((id: number) => id !== undefined);

  // 낙양과 장안만 레벨 10 (경)으로 설정
  let levelId = levelMap[scale] || 6;
  if (scale === '특' && (name === '낙양' || name === '장안')) {
    levelId = 10;
  }
